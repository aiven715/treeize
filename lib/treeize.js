...
Treeize.prototype.signature = function (row, options, auto) {
  ...
  for (var key in row) {
    ...
    attr.path = attr.split
      .slice(0, attr.split.length - 1)
      .join(opt.input.delimiter);
    attr.parent = attr.split
      .slice(0, attr.split.length - 2)
      .join(opt.input.delimiter); //.replace(/^[\*\-\+]|[\*\-\+]$/g,'')
    ...
    node.isCollection =
      !attr.node || (opt.input.detectCollections && isPlural(attr.node));
...
Treeize.prototype.grow = function (data, options) {
  ...
  if (!signature.nodes.length) {
    const extraPaths = data[0].reduce((acc, props) => {
      const path = props.split(this.baseOptions.input.delimiter);
      if (path.length === 1) {
        return acc;
      }
      return path.reduce((acc, key, i) => {
        const p = path
          .slice(0, -(i + 1))
          .join(this.baseOptions.input.delimiter);
        if (!data[0].includes(p) && !acc.includes(p) && i !== path.length - 1) {
          return [...acc, p];
        }
        return acc;
      }, acc);
    ...
    const extraValues = extraPaths.map((path) => {
      const last = path.split(this.baseOptions.input.delimiter).slice(-1)[0];
      if (isPlural(last)) {
        return [];
      }
      return {};
    ...
    if (extraPaths.length && extraValues.length) {
      data = data.map((row, i) => {
        if (i === 0) {
          return [...row, ...extraPaths];
        }
      ...
      if (!opt.output.prune || !isEmpty(blueprintExtended)) {
        ...
        if (opt.output.resultsAsObject && node.depth === 0) {
          ...
          if (node.isCollection) {
            ...
            if (!trail) {
              // do something to fix a broken trail (usually from too deep?)
              ...
              while (!(trail = trails[pathAttempt])) {
                segmentsStripped++;
                pathAttempt = segments
                  .slice(0, numSegments - segmentsStripped)
                  .join(":");
              ...
              segments
                .slice(numSegments - segmentsStripped)
                .forEach(function (segment) {
                  var isCollection =
                    (isPlural(segment) || segment.match(/^\+|\+$/)) &&
                    !segment.match(/^\-|\-$/);
            ...
            if (
              !trail[node.name] ||
              (opt.output.objectOverwrite &&
                typeof trail[node.name] !== typeof blueprintExtended)
            ) {
